...
const advertModel = {
     ...
     queries: {
          ...
          getUserAdverts: "select a.ad_id, a.ad_name, a.description, a.ad_image, a.ad_images, a.ad_type, a.ad_price, date_format(a.ad_date,'%Y-%m-%d') as ad_date, a.status, a.contact, a.ad_views, c.sub_id, c.sub_name, p.plan_name, u.full_name, u.user_location, u.profile_image,u.user_phone, u.user_email, u.rating, category.category_name, category.category_id from adverts a inner join users u on a.ad_user_id = u.user_id inner join sub_category c on a.sub_category_id = c.sub_id inner join payment_plan p on u a.ad_plan_id = p.plan_id inner join category  on c.parent_id = category.category_id where u.user_id = ?;",
     ...
     findAll: async(req, res) => {
          try{
               ...
               await Promise.all([
                    ...
                    info.page ?
                    new Promise(resolve => {
                         db.query(advertModel.queries.findAllPaged,[info.page], (err, data) => {
                              if(err){
                                   adsFetched = [];
                              }
                              adsFetched = data;
               ...
               return res.json({status:"pass", message:"success",data: {generalAds:adsFetched[0] ? adsFetched : "no data found", boostedAds}, totalAds});
          }catch(error){
               return res.json({status:"fail", message:"server error", error});
          }
     },
