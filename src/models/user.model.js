...
const userModel = {
     ...
     queries: {
          ...
          getBestViewedUsers: "select u.user_id, u.username, u.full_name, u.profile_image, u.user_phone, sum(a.ad_views) as total_ads from users u inner join adverts a on u.user_id = a.ad_user_id group by u.user_id having total_ads > 50 order by total_ads desc;",
          getUserAdsTotal: "select count(*) as total_ads from adverts where ad_user_id = ?",
          getBestSellers: "select u.user_id, u.username, u.full_name,u.profile_image,  u.user_phone, p.plan_name, sum(a.ad_views) as total_ads from users u inner join payment_plan p on u.ad_plan_id = p.plan_id inner join adverts a on u.user_id = a.ad_user_id group by u.user_id where p.plan_name like '%premium%' or p.plan_name like '%urgent%' or p.plan_name like '%featured%';",
